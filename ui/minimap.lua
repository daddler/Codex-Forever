...
-- Steinrings, Zoom mit dem Mausrad, darueber das Gebiet, darunter
-- Koordinaten und Uhrzeit.
--
...
local frame, zone, coords, clock, border
...
local function UpdateTexts()
    ...
    if Opt("coords") then
        ...
        coords:SetText(x and (string.format("%.1f · %.1f", x * 100, y * 100):gsub("%.(%d)", ",%1")) or "–")
        coords:Show()
    else
        coords:Hide()
    end
...
local function Enable()
    ...
    zone = frame:CreateFontString(nil, "OVERLAY")
    zone:SetPoint("BOTTOM", mm, "TOP", 0, 4)
    zone:SetWordWrap(false)
    zone:SetTextColor(unpack(C.textBright))
    coords = frame:CreateFontString(nil, "OVERLAY")
    coords:SetPoint("BOTTOMLEFT", mm, "BOTTOMLEFT", 4, 4)
    coords:SetTextColor(unpack(C.textNormal))
    clock = frame:CreateFontString(nil, "OVERLAY")
    clock:SetPoint("BOTTOMRIGHT", mm, "BOTTOMRIGHT", -4, 4)
    ...
    ev:SetScript("OnEvent", UpdateTexts)
    Apply()
end
    ...
    pages = {
        { key = "allgemein", label = "Allgemein", build = function(B)
            ...
            B:Row({ type = "color", label = "Randfarbe", key = "borderColor",
                  ...
                  { type = "empty" })
            B:Section("Anzeigen")
            B:Row({ type = "toggle", label = "Gebiet darüber", key = "zoneText" },
