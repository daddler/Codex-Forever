...
function WeintCodex.Signup.SetOverride(originalName, newName)
...
function WeintCodex.Signup.GetOverride(originalName)
...
function WeintCodex.Signup.IsResolved(p)
...
-- Rosters aussperrt.
function WeintCodex.Signup.HasLineup(dayData)
...
function WeintCodex.Signup.ShouldInvite(p, hasLineup)
...
function WeintCodex.Signup.StatusLabel(p, hasLineup)
...
function WeintCodex.Signup.Freshness(data)
...
function WeintCodex.Signup.CountUnresolved(data)
    if not data or not data.players then return 0 end

    local count = 0

    for _, p in ipairs(data.players) do
        if not WeintCodex.Signup.IsResolved(p) then
            count = count + 1
        end
    end
...
function WeintCodex.Signup.ResolveNames(data)
    ...
    for _, p in ipairs(data.players) do
        ...
        local override = WeintCodex.Signup.GetOverride(p.originalName)
        if override then
            p.name = override
        end
    end
...
StaticPopupDialogs["WEINTCODEX_EDIT_ROSTER_NAME"] = {
    text = "Charaktername für '%s' eingeben:\n|cff8A8A98(Crossrealm: Name-Realm, z. B. Njiah-OokOok)|r",
    button1 = "Speichern",
    button2 = "Abbrechen",
    hasEditBox = true,
    maxLetters = 48,
    ...
    OnAccept = function(self, data)
        ...
        if newName ~= "" and data and data.originalName then
            WeintCodex.Signup.SetOverride(data.originalName, newName)
...
local function UpdateInspector(raidData)
    ...
    local open  = WeintCodex.Signup.CountUnresolved(raidData)

    local freshText, freshTone = "—", "textDim"

    if raidData then
        freshText, freshTone = WeintCodex.Signup.Freshness(raidData)
...
local function RefreshRaidDisplay(raidData)
    ...
    local freshness, tone = WeintCodex.Signup.Freshness(raidData)
    ...
    -- je Zeile.
    local hasLineup = WeintCodex.Signup.HasLineup(raidData)
    ...
    local function DrawSection(players, sectionLabel, colorName)
        ...
        for _, p in ipairs(players) do
            ...
            local resolved = WeintCodex.Signup.IsResolved(p)
            ...
            local stateLabel, stateTone =
                WeintCodex.Signup.StatusLabel(p, hasLineup)
            ...
            editBtn:SetScript("OnClick", function()
                StaticPopup_Show("WEINTCODEX_EDIT_ROSTER_NAME", p.originalName, nil, {
                    ...
                    refresh = function()
                        ...
                        WeintCodex.Signup.ResolveNames(raidData)
...
function WeintCodex.Signup.Show()
    local cp = WeintCodex.ContentPanel
...
function WeintCodex.Signup.RefreshDay(day, data)
...
function WeintCodex.Signup.Refresh(data)
