...
local function SaveIngameCalendarEvent(draft, statusCallback)
    ...
    if confirmed then
        ...
        if #missing > 0 then
            msg = msg .. "\n|cffF46366Vom Server nicht gefunden:|r "
        ...
        msg = msg .. "Angefragt: " .. total ..
            "\n|cff8A8A98Der Client gibt die Einladungsliste nicht her - " ..
...
function WeintCodex.Calendar.Dump()
    ...
    if inviteFnName then
        say("Einladungsfunktion: |cff34C77B" .. inviteFnName .. "|r")
    else
        say("|cffF46366Keine Einladungsfunktion gefunden.|r Gesucht wurde nach:")
        for _, entry in ipairs(INVITE_FUNCS) do
            say("   " .. entry.name)
        end
    end
    ...
    for _, key in ipairs({
        ...
        table.insert(api, (have and "|cff34C77B" or "|cffF46366") .. key .. "|r")
    end
    ...
    for _, key in ipairs({ "raidWednesday", "raidThursday" }) do
        ...
        if not data or not data.players or #data.players == 0 then
            ...
            local hasLineup = WeintCodex.Signup.HasLineup(data)

            say(key .. ": " .. #data.players .. " Zeilen, Aufstellung "
                .. (hasLineup and "angekuendigt" or "nicht angekuendigt"))

            for _, p in ipairs(data.players) do
                local invited = WeintCodex.Signup.ShouldInvite(p, hasLineup)
...
local function CreateCalendarFrame()
    ...
    autoFillLbl:SetText("|cff8A8A98" .. WeintCodex.Icon("Interface\\Icons\\INV_Misc_PocketWatch_01", 14) .. "  Felder aus Raidanmeldung befüllen|r")
    ...
    autoFillBtn:SetScript("OnClick", function()
        ...
        if data and data.date and data.date ~= "" then
            ...
            f.StatusText:SetText("|cffF46366" .. WeintCodex.Icon("Interface\\Icons\\INV_Misc_QuestionMark", 14) .. " Keine Raidanmeldung für " ..
    ...
    local cbTank   = MakeCheckbox(leftPanel, "|cff4EA8F5" .. WeintCodex.Icon("Interface\\Icons\\Ability_Warrior_DefensiveStance", 14) .. "  Tanks einladen|r",   16, -284, true)
    local cbHealer = MakeCheckbox(leftPanel, "|cff34C77B" .. WeintCodex.Icon("Interface\\Icons\\Spell_Holy_Renew", 14) .. "  Heiler einladen|r",  16, -308, true)
    local cbDps    = MakeCheckbox(leftPanel, "|cffF46366" .. WeintCodex.Icon("Interface\\Icons\\Ability_DualWield", 14) .. "  DPS einladen|r",     16, -332, true)
    ...
    C_Timer.After(0.2, function()
        ...
        if ok then
            apiInfo:SetText("|cff34C77B" .. WeintCodex.Icon("Interface\\RaidFrame\\ReadyCheck-Ready", 14) .. " Kalender-API (C_Calendar) verfügbar|r")
        else
            apiInfo:SetText("|cffF46366" .. WeintCodex.Icon("Interface\\Icons\\INV_Misc_QuestionMark", 14) .. " Kalender-API nicht erkannt – manuelles Erstellen nötig|r")
        end
        f.ApiAvailable = ok
    ...
    local function WatchDraft()
        ...
        if #blocked > 0 and #blocked >= total and total > 0 then
            f.CreateBtnLbl:SetText("|cffffffff"
                ...
                .. "  Eintrag speichern|r")
            f.StatusText:SetText("|cffF46366"
        ...
        if #missing == 0 then
            f.StatusText:SetText("|cff34C77B"
    ...
    createBtn:SetScript("OnClick", function()
        ...
        if f.Draft then
            ...
            SaveIngameCalendarEvent(draft, function(success, msg)
                if success then
                    ...
                    if #benched > 0 then
                        msg = msg .. "\n|cff8A8A98Nicht in der Aufstellung ("
                    ...
                    f.StatusText:SetText("|cffF46366" .. msg .. "|r")
                end
            end)
        ...
        if title == "" then
            f.StatusText:SetText("|cffF46366" .. WeintCodex.Icon("Interface\\RaidFrame\\ReadyCheck-NotReady", 14) .. " Bitte einen Titel eingeben.|r")
            return
        end
        ...
        -- sechsten Feld, siehe WeintCodex.Signup.IsResolved). Ingame
        ...
        local function AddInvitees(dayData)
            if dayData and dayData.players then
                local hasLineup = WeintCodex.Signup
                    and WeintCodex.Signup.HasLineup
                    and WeintCodex.Signup.HasLineup(dayData)
                    ...
                    if not seen[nameKey] then
                        ...
                        if include then
                            ...
                            local invited = f.CbAll:GetChecked()
                                or not (WeintCodex.Signup and WeintCodex.Signup.ShouldInvite)
                                or WeintCodex.Signup.ShouldInvite(p, hasLineup)

                            if not invited then
                                table.insert(benched, p.name)
                            elseif WeintCodex.Signup and WeintCodex.Signup.IsResolved
                               and not WeintCodex.Signup.IsResolved(p) then
        ...
        local draft = PrepareIngameCalendarEvent(
            ...
            function(success, msg)
                f.StatusText:SetText(
                    (success and "|cff34C77B" or "|cffF46366") .. msg .. "|r")
            end
        )
...
RefreshPlayerPreview = function(f, raidData)
    ...
    for _, item in ipairs(players) do
        ...
        local resolved = not (WeintCodex.Signup and WeintCodex.Signup.IsResolved)
            or WeintCodex.Signup.IsResolved(p)
        ...
        local ccol = classColors[p.class] or "|cffEDEDF2"
        ...
        if resolved and item.invited then
            ...
            nameLbl:SetText("|cff5F5F6B" .. (p.name or "?") .. "|r")
        end
        ...
        if WeintCodex.Signup and WeintCodex.Signup.StatusLabel then
            label, colorName = WeintCodex.Signup.StatusLabel(p, item.hasLineup)
        end
        ...
        elseif label then
            ...
            classLbl:SetText("|cff5F5F6B" .. (p.class or "") .. "|r")
        end
    ...
    local countText =
        "|cff4EA8F5" .. tanks .. "T|r  " ..
        "|cff34C77B" .. healers .. "H|r  " ..
        "|cffF46366" .. dps .. "D|r  " ..
        "|cff8A8A98" .. invitable .. " von " .. total .. "|r"
    ...
    if excluded > 0 then
        countText = countText ..
            "  |cff8A8A98" .. excluded .. " nicht aufgestellt|r"
    end
    ...
    if WeintCodex.Signup and WeintCodex.Signup.Freshness then
        local freshness, tone = WeintCodex.Signup.Freshness(stampSource)
...
function WeintCodex.Calendar.Show()
    ...
    f.CreateBtnLbl:SetText(
        (mayInvite and "|cffffffff" or "|cff8A8A98")
