...
--------------------------------------------------
-- Beobachtungsliste und Sollbestaende
--------------------------------------------------
-- DIESE LISTE IST LEER, UND DAS IST EIN ZUSTAND, KEIN RUECKSTAND.
--
-- In der Fassung fuer Mists of Pandaria standen hier rund sechzig
-- Posten mit Sollbestaenden - Kraeuter, Fischfleisch, Faesschen,
-- Traenke, Sockelsteine. Kein einziger davon sagt ueber Forever etwas
-- aus: welche Verbrauchsgueter es gibt, welche ein Raid braucht und in
-- welcher Menge, ist nicht veroeffentlicht. Sie stehen zu lassen waere
-- nicht "schon mal was" gewesen, sondern eine Behauptung: eine
-- Gildenbank, die 0 von 500 Goldlotus fuehrt, ist nach dieser Liste
-- leer - dabei kennt Forever die Pflanze gar nicht.
--
-- Solange die Liste leer ist, arbeitet der Gildenbankscan OHNE
-- Beobachtungsliste: er nimmt auf, was wirklich in den Faechern liegt,
-- und setzt KEIN Soll (siehe UpdateRequiredMaterialsFromCache). Ein
-- Posten ohne Soll bekommt in der Anzeige keinen Statuspunkt und keinen
-- Balken - "kein Sollbestand hinterlegt" ist etwas anderes als "unter
-- dem Soll".
--
-- EINTRAGEN, SOBALD DIE GILDE IHRE SOLLBESTAENDE KENNT:
--
--   { name = "<exakter Itemname>", count = 0,
--     category = "<Rubrik in der Seitennavigation>", target = <Soll> },
--
-- Sobald hier ein einziger Posten steht, faellt der Scan zurueck auf
-- "nur diese Posten" - das ist Absicht: eine halb gefuellte Liste ist
-- eine Auswahl, keine Luecke.
--------------------------------------------------
...
-- Rueckwaertsname: die Anzeige unten spricht die Liste an mehreren
-- Stellen als Datenquelle an.
local sampleData = WATCHLIST
...
local function RefreshMatDisplay(matData, filterCat)
    ...
    if isSample then
        f.UpdateStr:SetText(WeintCodex.ColorText("textFaint",
            "Noch kein Gildenbankscan — Gildenbank öffnen oder über ")
            .. WeintCodex.ColorText("purple", "Import")
            .. WeintCodex.ColorText("textFaint", " einspielen"))
    else
    ...
    if #items == 0 then
        ...
        for _, item in ipairs(items) do
            ...
            -- OHNE SOLLBESTAND GIBT ES KEINEN ZUSTAND. Bis 3.x rechnete
            -- diese Stelle `pct = 0`, wenn kein Soll hinterlegt war, und
            -- jeder solche Posten stand danach rot als Engpass da. Fuer
            -- Forever ist das der Normalfall und nicht die Ausnahme (siehe
            -- WATCHLIST oben): `statusColor = nil` heisst hier "keine
            -- Angabe", der Punkt wird dann nur als Kontur gezeichnet und
            -- der Posten zaehlt in keiner der drei Kennzahlen mit.
            local statusColor, pct
            if target > 0 then
                pct = amount / target
            ...
            local sColor = C[statusColor or "textFaint"]
            ...
            local brightName = statusColor == "success" and "successBright"
                or (statusColor == "warning" and "warningBright"
                    or (statusColor == "danger" and "dangerBright" or "textMuted"))
            if target > 0 then
...
local function UpdateRequiredMaterialsFromCache()
    ...
    if #WATCHLIST.items > 0 then
        ...
        -- Mit Beobachtungsliste: genau diese Posten, mit ihrem Soll. Was
        -- nicht auf der Liste steht, interessiert nicht - auch dann nicht,
        -- wenn es in der Bank liegt.
        for _, item in ipairs(WATCHLIST.items) do
            ...
            table.insert(newData.items, {
                ...
                target   = item.target,
                stats    = item.stats,
                note     = item.target and string.format("%d/%d", count, item.target) or nil,
            })
        end

    else

        -- Ohne Beobachtungsliste: was wirklich in den Faechern liegt, und
        -- KEIN Soll. Der Unterschied ist der ganze Punkt - eine Bestandsliste
        -- ohne Soll sagt "so viel ist da", eine mit Soll sagt zusaetzlich
        -- "so viel fehlt". Das zweite koennen wir fuer Forever nicht sagen.
        local byName = {}
        for tabIndex, tabData in pairs(WeintCodex.SavedData.guildBankCache) do
            for _, item in ipairs(tabData.items or {}) do
                local key = item.name:lower()
                if not byName[key] then
                    byName[key] = item.name
                end
            end
        end

        local names = {}
        for key, display in pairs(byName) do
            names[#names + 1] = { key = key, name = display }
        end
        ...
        for _, entry in ipairs(names) do
            table.insert(newData.items, {
                name     = entry.name,
                count    = tostring(totals[entry.key] or 0),
                category = "Gildenbank",
                target   = nil,
            })
        end
