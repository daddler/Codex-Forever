...
-- Schlachtzugs-Lockout ist an den einzelnen Charakter gebunden:
...
--
-- Die Module, die Fortschritt anzeigen (aktuell modules/raidpages.lua),
...
-- --------------------------------------------------
-- Welche Schlachtzug-IDs traegt dieser Charakter gerade?
--
-- Direkt aus der Lockout-API, ohne Umweg ueber den eigenen
-- Fortschrittsspeicher: die Uebersicht und die Schlachtzugsseite
-- fragen "habe ich diese Woche schon", und diese Frage beantwortet
-- der Server, nicht unser Zaehler.
--
-- Zugeordnet wird ueber den Namen des Schlachtzugs aus
-- data/raids.lua. Die Namen von Forever sind englisch ("Hyjal
-- Summit"), der Client liefert sie lokalisiert - deshalb wird
-- BEIDES probiert, und was nicht passt, faellt still heraus. Eine
-- falsche Zuordnung waere schlimmer als keine: sie zeigte eine ID
-- auf einem Schlachtzug an, den man nie betreten hat.
--
-- Rueckgabe: { [<raid.id>] = { name, reset, difficulty, encounters } }
-- Eine LEERE Tabelle heisst "keine gespeicherte ID" - fehlt die API
-- ganz, kommt ebenfalls eine leere Tabelle, und die Oberflaeche sagt
-- dann nichts ueber IDs statt "keine".
-- --------------------------------------------------

function WeintCodex.EncounterTracking.SavedLockouts()
    local out = {}

    local raids = WeintCodex.RaidData and WeintCodex.RaidData.All()
    if type(raids) ~= "table" then return out end

    local numSaved = SafeCall(GetNumSavedInstances)
    if type(numSaved) ~= "number" then return out end

    for i = 1, numSaved do
        local name, _, reset, difficultyID, locked, extended, _, isRaid, _, difficultyName, numEncounters =
            SafeCall(GetSavedInstanceInfo, i)

        if type(name) == "string" and (locked or extended) and isRaid
            and not IsLFR(difficultyID, difficultyName) then

            local lower = name:lower()

            for _, raid in ipairs(raids) do
                if lower:find(raid.name:lower(), 1, true) then
                    -- Mehrere IDs derselben Instanz (Groessen,
                    -- Schwierigkeiten): die mit der laengsten Restzeit
                    -- gewinnt, weil sie die ist, die noch bindet.
                    local previous = out[raid.id]
                    if not previous or (tonumber(reset) or 0) > (previous.reset or 0) then
                        out[raid.id] = {
                            name       = name,
                            reset      = tonumber(reset) or 0,
                            difficulty = difficultyName,
                            encounters = tonumber(numEncounters),
                        }
                    end
                end
            end
