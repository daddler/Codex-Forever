...
local tabs = {
    ...
    -- wuerde ausserdem genau den Weg versperren, ueber den man abstellt, was
    -- einen gestoert hat.
...
function WeintCodex.Navigation.SwitchTo(tabId)
    ...
    if tabId == "uebersicht" then
        -- Die Startseite raeumt selbst auf, deshalb genuegt der direkte Aufruf.
...
-- Umlaut in der Mitte auf, und der Client zeichnet den Rest als leeres
-- Kaestchen.
...
function WeintCodex.ShowHome()
    ...
    local COL_TOP = PAD_Y + 76 + 116 + GAP
    ...
    -- Zeile mit Punkt links, Text, Kennwert rechts. `dotTone = nil` zeichnet
    -- den Punkt als blosse Kontur - "keine Angabe" ist kein Zustand.
    ...
    CardHeader(prep, "Deine Ausrüstung",
        ...
        (not gear) and "textMuted" or (openIssues > 0 and "danger" or "success"))

    local specLine = WeintCodex.Eyebrow(prep,
        (gear and gear.specDisplay) or "Spezialisierung noch nicht bekannt",
        { size = 10 })
    ...
    if not gear then
        ...
        y = StateRow(prep, y, nil, "Ausrüstung konnte nicht gelesen werden", nil)
    else
        local shown = 0
        for _, slot in ipairs(gear.empty or {}) do
            if shown >= 4 then break end
            y = StateRow(prep, y, "danger", Ellipsis(slot, 42), "leer", "dangerBright")
            shown = shown + 1
        end
        for _, slot in ipairs(gear.broken or {}) do
            if shown >= 4 then break end
            y = StateRow(prep, y, "warning", Ellipsis(slot, 42), "defekt", "warningBright")
            shown = shown + 1
        end
        if shown == 0 then
            y = StateRow(prep, y, "success", "Alle Plätze belegt und heil",
                         "ok", "successBright")
        end
        if gear.itemLevel then
            y = StateRow(prep, y, nil, "Gegenstandsstufe angelegt",
                         string.format("%.1f", gear.itemLevel), "textNormal")
        end
    end

    local prepBtn = WeintCodex.CreateButton(prep, {
        text = "Charakter öffnen", kind = "secondary", backdrop = "cardBottom",
...
function WeintCodex.ResetToHome()
    -- Die Uebersicht ist ein eigener Navigationspunkt. Ohne das Zuruecksetzen
    -- bliebe der zuletzt besuchte Eintrag markiert, waehrend rechts die
    -- Startseite steht - und ein Klick auf "Uebersicht" wuerde nichts tun,
    -- weil activeTab noch auf dem alten Eintrag stuende.
    for _, b in ipairs(tabButtons) do SetTabActive(b, false) end
